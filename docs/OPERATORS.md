...
`$not` takes a single nested query `Map` and inverts its result under Strong Kleene logic: a nested MATCHED becomes NOT_MATCHED and vice versa, while a nested UNDETERMINED stays UNDETERMINED (¬unknown = unknown). `$and`/`$or` take a `List` of nested query maps and combine their tri-state results using Strong Kleene (K3) logic — so `$or` short-circuits to MATCHED on any match, and `$and` short-circuits to NOT_MATCHED on any non-match, even when some sub-queries are UNDETERMINED.
